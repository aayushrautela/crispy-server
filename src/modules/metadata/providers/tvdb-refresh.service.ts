...
import { TvdbCacheService } from './tvdb-cache.service.js';
...
export class TvdbRefreshService {
  constructor(
  ...
  async refreshIdentity(client: DbClient, profileId: string, identity: MediaIdentity): Promise<MetadataRefreshSummary> {
    ...
    if (identity.provider !== 'tvdb' || !identity.providerId) {
      summary.skipped += 1;
      return summary;
    }

    try {
      await this.tvdbCacheService.refreshTitleBundle(client, identity.providerId);
