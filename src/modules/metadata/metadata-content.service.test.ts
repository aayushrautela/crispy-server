...
test('getTitleContent requires MDBList key when not configured', async () => {
  const { MetadataContentService } = await import('./metadata-content.service.js');
  ...
  await assert.rejects(
    () => service.getTitleContent('user-1', 'movie:tmdb:55'),
    (error: unknown) => {
...
test('getTitleContent resolves MDBList content with valid key', async () => {
  ...
  const result = await service.getTitleContent('user-1', 'movie:tmdb:55');
