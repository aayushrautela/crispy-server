...
export class AccountSettingsRepository {
  ...
  async patchSettingsForUser(client: DbClient, userId: string, patch: Record<string, unknown>): Promise<Record<string, unknown>> {
    const result = await client.query(
      `
        INSERT INTO identity.account_preferences (account_id, settings_json, updated_at)
        VALUES ($1::uuid, $2::jsonb, now())
        ON CONFLICT (account_id)
        DO UPDATE SET
          settings_json = identity.account_preferences.settings_json || EXCLUDED.settings_json,
          updated_at = now()
  ...
  async setSecretForUser(client: DbClient, userId: string, fieldKey: string, value: string): Promise<void> {
    ...
    await client.query(
      `
        INSERT INTO private.account_secrets (account_id, secrets_json, updated_at)
        VALUES ($1::uuid, jsonb_build_object($2::text, $3::text), now())
        ON CONFLICT (account_id)
        DO UPDATE SET
          secrets_json = private.account_secrets.secrets_json || jsonb_build_object($2::text, $3::text),
          updated_at = now()
      `,
  ...
  async listSecretsForField(client: DbClient, fieldKey: string): Promise<AccountSecretRecord[]> {
    const result = await client.query(
      `
        SELECT account_id::text AS app_user_id,
