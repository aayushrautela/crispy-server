...
function mapTasteProfile(row: Record<string, unknown>): TasteProfileRecord {
  return {
    ...
    vectors: isRecord(row.vectors) && Object.keys(row.vectors).length > 0 ? row.vectors as TasteVectors : { schemaVersion: 1, genres: [], tags: [], people: [], mood: [], decades: [] },
  ...
  async upsert(client: DbClient, params: {
    ...
    const result = await client.query(
      ...
      [
        ...
        params.source,
        JSON.stringify(params.vectors ?? { schemaVersion: 1, genres: [], tags: [], people: [], mood: [], decades: [] }),
      ],
    );
