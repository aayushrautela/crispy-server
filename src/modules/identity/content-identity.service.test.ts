...
test('ensureContentIds falls back to individual materialization when batch misses refs', async () => {
  ...
  const identities = [
    ...
    inferMediaIdentity({ mediaType: 'show', provider: 'tvdb', providerId: 88 }),
  ];
  ...
  assert.equal(contentIds.get('show:tvdb:88'), 'content-show-88');
  assert.deepEqual(calls, [
    [
      { provider: 'tmdb', entityType: 'movie', externalId: '77' },
      { provider: 'tvdb', entityType: 'show', externalId: '88' },
    ],
    [{ provider: 'tmdb', entityType: 'movie', externalId: '77' }],
    [{ provider: 'tvdb', entityType: 'show', externalId: '88' }],
  ]);
...
test('ensureTitleContentIds falls back to individual materialization when batch misses refs', async () => {
  ...
  const contentIds = await service.ensureTitleContentIds({} as never, [
    ...
    { mediaType: 'show', provider: 'tvdb', providerId: 88 },
  ]);
...
test('ensureEpisodeContentIds falls back to individual materialization when batch misses refs', async () => {
  ...
  const contentIds = await service.ensureEpisodeContentIds({} as never, [
    { parentMediaType: 'show', provider: 'tvdb', parentProviderId: 44, seasonNumber: 1, episodeNumber: 2 },
    { parentMediaType: 'show', provider: 'tvdb', parentProviderId: 44, seasonNumber: 1, episodeNumber: 3 },
  ]);
  ...
  assert.deepEqual(calls, [
    [
      { provider: 'tvdb', entityType: 'episode', externalId: '44:s1:e2' },
      { provider: 'tvdb', entityType: 'episode', externalId: '44:s1:e3' },
    ],
    [{ provider: 'tvdb', entityType: 'episode', externalId: '44:s1:e2' }],
    [{ provider: 'tvdb', entityType: 'episode', externalId: '44:s1:e3' }],
  ]);
...
test('ensureSeasonContentIds falls back to individual materialization when batch misses refs', async () => {
  ...
  assert.deepEqual(calls, [
    [
      { provider: 'tvdb', entityType: 'season', externalId: '44:s1' },
      { provider: 'tvdb', entityType: 'season', externalId: '44:s2' },
    ],
    [{ provider: 'tvdb', entityType: 'season', externalId: '44:s1' }],
    [{ provider: 'tvdb', entityType: 'season', externalId: '44:s2' }],
  ]);
