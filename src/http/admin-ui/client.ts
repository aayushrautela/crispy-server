...
(() => {
  ...
  function bindHome() {
    ...
    root.querySelectorAll('[data-home-form]').forEach((form) => {
  ...
  function loadHomeProfilesEmpty() {
    const form = document.querySelector('[data-home-form="profile-mode"]');
    if (form) form.hidden = true;
  }

  function toggleHomeForm(name, show) {
    const form = document.querySelector('[data-home-form="' + name + '"]');
    if (form) form.hidden = !show;
  }

  function setHomeStatus(selector, message, isError) {
  ...
  async function inspectHomeProfile(form) {
    const data = new FormData(form);
    ...
    const preview = document.getElementById('home-profiles-preview');
    if (!preview) return;
    if (result.error) {
      setHomeStatus('#home-profiles-status', result.error, true);
      preview.innerHTML = '';
      return;
    }
    setHomeStatus('#home-profiles-status', 'Mode: ' + result.mode + ' / Source: ' + result.source, false);
    preview.innerHTML = renderHomeSections(result.sections || []);
    const modeForm = document.querySelector('[data-home-form="profile-mode"]');
    if (modeForm) {
  ...
  async function setHomeProfileMode(form) {
    const data = new FormData(form);
    const accountId = document.querySelector('[data-home-form="profile-lookup"] input[name="accountId"]');
    const profileId = document.querySelector('[data-home-form="profile-lookup"] input[name="profileId"]');
    ...
    try {
      ...
      setHomeStatus('#home-profiles-status', 'Home mode updated.', false);
      void inspectHomeProfile(document.querySelector('[data-home-form="profile-lookup"]'));
    } catch (error) {
      setHomeStatus('#home-profiles-status', error.message || 'Failed to update mode.', true);
    }
  }

  async function submitHomeRecompute() {
    const accountId = document.querySelector('[data-home-form="profile-lookup"] input[name="accountId"]');
    const profileId = document.querySelector('[data-home-form="profile-lookup"] input[name="profileId"]');
    if (!accountId || !profileId) return;
    try {
      await fetchJson(apiPath('/accounts/' + encodeURIComponent(accountId.value) + '/profiles/' + encodeURIComponent(profileId.value) + '/notify-recompute'), { method: 'POST' });
      setHomeStatus('#home-profiles-status', 'Notified recommender.', false);
    } catch (error) {
      setHomeStatus('#home-profiles-status', error.message || 'Failed to notify recommender.', true);
    }
  }
  ...
  function renderHomeSections(sections) {
    ...
    return sections.map((section) => '<div class="section-card">'
      + '<h4>' + escapeHtml(section.title || section.listKey || 'Section') + '</h4>'
      + '<p class="section-meta">' + escapeHtml(String(section.items ? section.items.length : 0)) + ' items'
      + (section.sectionType ? ' · ' + escapeHtml(String(section.sectionType)) : '') + '</p>'
      + '</div>').join('');
  }
  ...
  async function loadDiagnostics(options) {
    ...
    try {
      const imports = await fetchJson(apiPath('/diagnostics/imports/connections?limit=8&refreshFailuresOnly=false'));
      const payload = { imports };
      state.diagnosticsPayload = payload;
      renderDiagnostics(payload.imports);
  ...
  function bindProfileActionButtons(accountId, profileId, container) {
    ...
    for (const button of recomputeButtons) {
      button.onclick = async () => {
        button.disabled = true;
        setMessage(messageEl, 'info', 'Notifying recommender...');
        try {
          await fetchJson(apiPath('/accounts/' + encodeURIComponent(accountId) + '/profiles/' + encodeURIComponent(profileId) + '/notify-recompute'), {
            method: 'POST',
          });
          setMessage(messageEl, 'success', 'Notified recommender.');
          pushNotification('success', 'Notified recommender', 'Notified recommender for profile ' + profileId + '.', true);
        } catch (error) {
          const description = describeApiError(error, 'Unable to notify recommender.');
          setMessage(messageEl, 'error', description);
          pushNotification('error', 'Notify failed', description, true);
        } finally {
          button.disabled = false;
        }
      };
  ...
  function renderDiagnostics(imports) {
    ...
    if (elements.backlogSummary) {
      elements.backlogSummary.textContent = 'Recommendation delivery is managed by the unified home pipeline.';
    }
  ...
  function renderOverviewSummary() {
    ...
    elements.overviewSummary.innerHTML = [
      ...
      statCard('Diagnostics', refreshFailures, 'items needing attention'),
    ].join('');
  }
