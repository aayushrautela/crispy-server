...
test('admin recommendation trigger rejects requests without the admin csrf header', async (t) => {
  const Fastify = (await import('fastify')).default;
  const { default: errorHandlerPlugin } = await import('../plugins/error-handler.js');
  const { default: adminUiAuthPlugin } = await import('../plugins/admin-ui-auth.js');
  const { registerAdminUiRoutes } = await import('./admin-ui.js');
  ...
  const response = await app.inject({
    method: 'POST',
    url: '/admin/api/accounts/test-account/profiles/test-profile/recommendations/start',
    headers: {
      cookie: sessionCookie,
      host: 'localhost',
      origin: 'http://localhost',
...
test('admin generation cleanup rejects requests without the admin csrf header', async (t) => {
  const Fastify = (await import('fastify')).default;
  const { default: errorHandlerPlugin } = await import('../plugins/error-handler.js');
  const { default: adminUiAuthPlugin } = await import('../plugins/admin-ui-auth.js');
  const { registerAdminUiRoutes } = await import('./admin-ui.js');
  ...
  const response = await app.inject({
    method: 'POST',
    url: '/admin/api/diagnostics/recommendations/generation-jobs/clear-blocked',
    headers: {
      cookie: sessionCookie,
      host: 'localhost',
      origin: 'http://localhost',
