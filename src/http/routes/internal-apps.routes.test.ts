...
async function buildServer(principal = buildPrincipal(), ownedProfiles: Array<{ accountId: string; profileId: string }> = [], serviceRecommendationListService?: ServiceRecommendationListService) {
  ...
  await registerInternalAppsRoutes(app, {
    ...
    profileSignalBundleService: { async getBundle() { return { accountId: 'acc-999', profileId: 'prof-888', purpose: 'recommendation-generation', eligibility: { eligible: true, eligibilityVersion: 1 }, bundle: { signalsVersion: 1, generatedAt: new Date('2024-01-01T00:00:00.000Z'), profileContext: { profileName: 'Test Profile', isKids: false, watchDataOrigin: 'server_sync' }, history: [{ Item: { Id: '00000000000040008000000000000101', Type: 'Movie', Name: 'Test Movie', OriginalTitle: null, Overview: null, Taglines: [], ProductionYear: 2024, PremiereDate: null, CommunityRating: null, OfficialRating: null, Certification: null, Genres: [], RunTimeTicks: null, Status: null, ProviderIds: { Tmdb: '550', Imdb: null, Tvdb: null }, ImageTags: { Primary: { small: null, medium: null, large: null }, Backdrop: [], Logo: null, Thumb: null, Screenshot: [] }, ParentImageTags: null, SeriesId: null, SeriesName: null, SeasonId: null, SeasonName: null, ParentIndexNumber: null, IndexNumber: null, AbsoluteIndexNumber: null, EpisodeTitle: null, AirDate: null, RemoteTrailers: [], PosterColor: null, BackdropColor: null, UserData: null }, watchedAt: new Date('2024-01-01T00:00:00.000Z'), progressPercent: 100, completionState: 'completed', durationSeconds: null }], ratings: [], watchlist: [], continueWatching: [] }, limits: {} }; } } satisfies ProfileSignalBundleService,
...
test('official recommender with accounts:all:read can access profile signals across accounts', async (t) => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(response.json().data.bundle.history[0].Item.Id, '00000000000040008000000000000101');
  assert.equal('itemId' in response.json().data.bundle.history[0], false);
});
