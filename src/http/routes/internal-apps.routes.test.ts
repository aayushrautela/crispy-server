...
async function buildServer(principal = buildPrincipal(), ownedProfiles: Array<{ accountId: string; profileId: string }> = [], serviceRecommendationListService?: ServiceRecommendationListService) {
  ...
  await registerInternalAppsRoutes(app, {
    ...
    profileSignalBundleService: { async getBundle() { return { accountId: 'acc-999', profileId: 'prof-888', purpose: 'recommendation-generation', eligibility: { eligible: true, eligibilityVersion: 1 }, bundle: { signalsVersion: 1, generatedAt: new Date('2024-01-01T00:00:00.000Z'), profileContext: { profileName: 'Test Profile', isKids: false, watchDataOrigin: 'server_sync' }, history: [{ itemId: '00000000000040008000000000000101', contentType: 'movie', watchedAt: new Date('2024-01-01T00:00:00.000Z'), progressPercent: 100, completionState: 'completed', durationSeconds: null }], ratings: [], watchlist: [], continueWatching: [] }, limits: {} }; } } satisfies ProfileSignalBundleService,
...
test('official recommender with accounts:all:read can access profile signals across accounts', async (t) => {
  ...
  assert.equal(response.statusCode, 200);
  assert.equal(response.json().data.bundle.history[0].itemId, '00000000000040008000000000000101');
