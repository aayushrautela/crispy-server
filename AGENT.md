...
- A successful `verifyPin` sets a Redis-backed unlock flag (key `profile_unlock:{profileId}:{authSubject}`, 30-day TTL) — Netflix model: verify once, stay unlocked until explicit lock. Profile-scoped routes (`/v1/profiles/:profileId/watch/*`, `/ai/*`, `/calendar*`) enforce the `requireProfileUnlock` guard when the target profile has a PIN: locked → `423 PROFILE_LOCKED`; no PIN → open. `POST /v1/profiles/:profileId/lock` clears the flag. Setting/changing/removing a PIN also re-locks. Clients never handle tokens.
...
- Profile `avatar_url` must be one of the built-in avatar ids (e.g. `toon_1`, `vibrent_7`); the catalog and validation live in `src/modules/profiles/avatars.ts`, and the PNG is served at `GET /v1/avatars/:id`. Selection is mandatory on signup and when creating a profile.
