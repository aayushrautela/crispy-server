...
- A successful `verifyPin` returns a short-lived HS256 **unlock token** (signed with `AUTH_JWT_SECRET`, TTL 15 min) binding profile id + authenticated user. Profile-scoped routes (`/v1/profiles/:profileId/watch/*`, `/ai/*`, `/calendar*`) enforce the `requireProfileUnlock` guard when the target profile has a PIN: no token → `423 PROFILE_LOCKED`, bad/expired/mismatched token → `403 INVALID_UNLOCK_TOKEN`. Present the token via `x-profile-unlock-token` header. Profiles without a PIN are open.
