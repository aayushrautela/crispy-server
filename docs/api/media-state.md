...
- Artwork follows the responsive-set convention: `artwork` resolves from the show (backdrop preferred, poster as fallback); `still` is the episode still when one exists. Logo artwork remains nullable; clients should fall back to text titles.
