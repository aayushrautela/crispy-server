...
- Resolution: in recommended mode, a `GET /home` response carries the profile's `reco` rails followed by the generic default rails flagged `show_with_reco`; profiles without reco rows get the full shared default alone. `custom` mode serves only the profile's `custom` snapshot. `source` reports the lead layer. Sources are never independently layered, and there is no cross-source dedup.
...
- A single `GET /home` response is reco rails + marked generic default rails in that order, or the full shared default alone, or the `custom` snapshot alone; no cross-source dedup.
